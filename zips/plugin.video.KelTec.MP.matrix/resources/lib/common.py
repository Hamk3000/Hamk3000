...
def OpenURL(url, headers={}, user_data={}, cookieJar=None, justCookie=False):
    ...
    if 'User-Agent' not in req.headers or req.headers['User-Agent'] == '':
        req.add_header('User-Agent', UA)
        req.add_header('Add-on', 'KelTec-Play-Tv')
        req.add_header('Plugin', 'Video') 
        req.add_header('Direitos', 'Reservados') 
        req.add_header('Team', 'KelTec-Media-Play') 
        req.add_header('Referer', 'Top Vip') 
        req.add_header('coppyright', '2021')         
